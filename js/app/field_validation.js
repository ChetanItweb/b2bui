...
function mobile_validate(id){
  var pass1 = document.getElementById(id).value;
   if(!pass1.replace(/\s/g, '').length){
      ...
      error_msg_alert('More than 20 digits are not allowed');
      $('#'+id).css({'border':'1px solid red'}); 
       document.getElementById(id).value="";
      $('#'+id).focus();
      g_validate_status = false;
    ...
    else if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(pass1) || /^(?:(?:https?|ftp):\/\/)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/.test(pass1))  { 
...
function validate_amount(id)
{
    ...
    if (iKeyCode != 46 && iKeyCode > 31 && (iKeyCode < 48 || iKeyCode > 57))
      {
        error_msg_alert('Please enter valid monthly target.');
        $('#'+id).css({'border':'1px solid red'});  
        document.getElementById(id).value="";
        $('#'+id).focus();
        g_validate_status = false;
...
{
 /*var pass1 = document.getElementById(id).value;  
  if( /^[a-zA-Z0-9]+$/.test(pass1) && pass1.length <= 21)
  {
    $('#'+id).css({'border':'1px solid #ddd'});
    return true;
  }
...
//function for valid date terrif
...
function ValidateIPaddress(inputText)
 {
    ...
    var ipAdresses = ipAddress.split(',');
    for(var i=0;i<ipAdresses.length;i++){
      if(!/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(ipAdresses[i])){
     // if(!ipAdresses[i].match(ipformat)){
          error_msg_alert('You have entered Invalid IP Address->'+ipAdresses[i]);
