...
public function customer_master_save()
{
  ...
  if($company_count>0){
	...
	$sq_visa = mysql_query("insert into customer_master (customer_id,type,first_name, middle_name, last_name, gender, birth_date, age, contact_no,landline_no, email_id,alt_email,company_name, address, address2, city, active_flag, created_at,service_tax_no,state_id,pan_no, branch_admin_id) values ('$customer_id','$cust_type', '$first_name', '$middle_name', '$last_name', '$gender', '$birth_date', '$age', '$contact_no','$landline_no', '$email_id','$alt_email_id','$company_name', '$address','$address2','$city', '$active_flag', '$created_at', '$service_tax_no','$state','$cust_pan','$branch_admin_id')");
...
public function customer_master_update(){
  ...
  if($company_count>0){
	...
	$sq_visa = mysql_query("update customer_master set type = '$cust_type',first_name='$first_name', middle_name='$middle_name', last_name='$last_name', gender='$gender', birth_date='$birth_date', age='$age', contact_no='$contact_no',landline_no = '$landline_no', email_id='$email_id',alt_email = '$alt_email_id',company_name = '$company_name', address='$address', address2='$address2', city='$city', active_flag='$active_flag', service_tax_no='$service_tax_no1', state_id='$state', pan_no ='$cust_pan' where customer_id='$customer_id'");
    ...
    if(empty($handle) === false) {
        ...
        while(($data = fgetcsv($handle, ",")) !== FALSE){
            ...
            if($count>0){
                ...
                $sq_max_id = mysql_fetch_assoc(mysql_query("select max(customer_id) as max from customer_master"));

                $customer_id = $sq_max_id['max']+1;
          ...
          if( !empty($cust_type) && preg_match('/^[a-zA-Z \s]*$/', $cust_type) && !empty($first_name) && preg_match('/^[a-zA-Z \s]*$/', $first_name) && preg_match('/^[a-zA-Z \s]*$/', $last_name) &&  preg_match('/^[a-zA-Z \s]*$/', $gender) && preg_match('/^[0-9 \s]{6,20}+$/', $contact_no)  && preg_match('/^[0-9]*$/', $state_id))
            {
              $validCount++;
              $sq_max = mysql_fetch_assoc(mysql_query("select max(customer_id) as max from customer_master"));
              $customer_id = $sq_max['max'] + 1;
              ...
              $sq_cust = mysql_query("insert into customer_master (customer_id,branch_admin_id, type,first_name, middle_name, last_name, gender, birth_date, age, contact_no,landline_no, email_id,alt_email,company_name, address,address2,city, active_flag, created_at,service_tax_no,state_id,pan_no) values ('$customer_id', '$branch_admin_id', '$cust_type', '$first_name', '$middle_name', '$last_name', '$gender', '$birth_date1', '$age', '$contact_no','$landline_no', '$email_id','$alt_email_id','$company_name', '$address', '$address2', '$city', 'Active', '$created_at', '$service_tax_no','$state_id','$pan_no')");
        ...
        header("Expires: 0");
            $output = fopen($save, "w");  
            fputcsv($output, array('Customer Type' , 'First Name' , 'Middle Name' , 'Last Name' , 'Gender' , 'Birthdate' , 'Age' , 'Contact No' ,'Email Id' , 'Alternative Email ID' , 'Company Name' , 'Landline No' , 'Address' , 'Address2' , 'City_id' , 'State_id' , 'Tax No' , 'PAN No'));  
