...
public function payment_save()
{
	...
	$sq_payment = mysql_query("insert into forex_booking_payment_master (payment_id, booking_id, branch_admin_id, financial_year_id, payment_date, payment_amount, payment_mode, bank_name, transaction_id, bank_id, clearance_status) values ('$payment_id', '$booking_id', '$branch_admin_id', '$financial_year_id', '$payment_date', '$payment_amount', '$payment_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status') ");
	if(!$sq_payment){
		$GLOBALS['flag'] = false;
...
public function finance_save($payment_id, $branch_admin_id)
{
    ...
    $payment_particular = get_sales_paid_particular(get_forex_booking_payment_id($booking_id,$yr1), $payment_date, $payment_amount1, $customer_id, $payment_mode, get_forex_booking_id($booking_id,$yr1));
    $gl_id = $pay_gl;
    $payment_side = "Debit";
    ...
    $payment_amount = $payment_amount1;
    $payment_date = $payment_date;
    $payment_particular = get_sales_particular(get_forex_booking_payment_id($booking_id,$yr1), $payment_date, $payment_amount1, $customer_id);
    $gl_id = $cust_gl;
    $payment_side = "Credit";
    $clearance_status = "";
    $transaction_master->transaction_save($module_name, $module_entry_id, $transaction_id, $payment_amount, $payment_date, $payment_particular, $gl_id, $payment_side, $clearance_status, $row_spec,$branch_admin_id);  
...
public function bank_cash_book_save($payment_id, $branch_admin_id)
{
	...
	$sq_booking_info = mysql_fetch_assoc(mysql_query("select customer_id from forex_booking_master where booking_id='$booking_id'"));
	...
	$bank_id = $bank_id;
	$particular = get_sales_paid_particular(get_forex_booking_payment_id($payment_id,$yr1), $payment_date, $payment_amount, $sq_booking_info['customer_id'], $payment_mode, get_forex_booking_payment_id($booking_id,$yr1));
...
public function payment_update()
{
	...
	$sq_payment = mysql_query("update forex_booking_payment_master set booking_id='$booking_id', financial_year_id='$financial_year_id', payment_date='$payment_date', payment_amount='$payment_amount', payment_mode='$payment_mode', bank_name='$bank_name', transaction_id='$transaction_id', bank_id='$bank_id', clearance_status='$clearance_status' where payment_id='$payment_id' ");
	if(!$sq_payment){
		$GLOBALS['flag'] = false;
...
public function finance_update($sq_payment_info, $clearance_status1)
{
	...
	$sq_forex_info = mysql_fetch_assoc(mysql_query("select customer_id from forex_booking_master where booking_id='$booking_id'"));
	...
	if($payment_amount1 > $payment_old_value)
	{
		...
		$payment_amount = $payment_amount1;
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_forex_booking_id($booking_id,$yr1), $payment_date, $payment_amount1, $customer_id, $payment_mode, get_forex_booking_id($booking_id,$yr1));
		...
		$payment_date = $payment_date;
		$payment_particular = get_sales_particular(get_forex_booking_id($booking_id,$yr1), $payment_date, $balance_amount, $customer_id);
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = $cust_gl;
		$payment_side = "Credit";
		$clearance_status = "";
		...
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_forex_booking_id($booking_id,$yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_forex_booking_id($booking_id,$yr1));
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = $pay_gl;
		$payment_side = "Credit";
		$clearance_status = ($payment_mode!="Cash") ? "Pending" : "";
		...
		$payment_amount = $payment_old_value;
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_forex_booking_payment_id($booking_id,$yr1), $payment_date, $payment_old_value, $customer_id, $payment_mode, get_forex_booking_id($booking_id,$yr1));
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = $pay_gl;
		$payment_side = "Credit";
		$clearance_status = ($payment_mode!="Cash") ? "Pending" : "";
...
public function payment_email_notification_send($booking_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   $total_amount = $sq_forex_info['net_total'];

   $date = $sq_forex_info['created_at'];
   $yr = explode("-", $date);
   $year =$yr[0];

   $sq_customer_info = mysql_fetch_assoc(mysql_query("select email_id from customer_master where customer_id='$sq_forex_info[customer_id]'"));
   $email_id = $sq_customer_info['email_id'];

   $sq_total_amount = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from forex_booking_payment_master where booking_id='$booking_id'"));
   ...
   global $model;
   $model->generic_payment_mail('53',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id, $subject);
}
...
public function payment_update_email_notification_send($payment_id)
{
	...
	global $model;
   	$model->generic_payment_mail('63',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id, $subject, $update_payment);
