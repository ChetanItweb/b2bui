...
{ 
    ...
    $tour_cost_total = $_POST['tour_cost_total'];
    ...
    $this->package_tour_tranpsort_information_save($max_booking_id, $transp_vehicle_arr, $transp_start_date, $trans_end_date);

    //** This function stores the excursion information
    $this->package_tour_exc_information_save($max_booking_id, $exc_city_arr, $exc_name_arr);
    ...
    if($quotation_id == 0){
      ...
      $booking_save_transaction->finance_save($max_booking_id, $row_spec, $branch_admin_id);
      ...
      if($GLOBALS['flag']){
        ...
        $this->send_mail_to_traveler($max_booking_id, $tour_name, $tour_from_date, $tour_to_date, $customer_id, $email_id);
        ...
        $this->package_tour_payment_master_save($customer_id,$max_booking_id, $payment_date, $payment_mode, $payment_amount, $bank_name, $transaction_id, $payment_for, $p_travel_type, $bank_id_arr, $branch_admin_id , $emp_id);   
        ...
        $payment_master->payment_sms_notification_send($max_booking_id, $total_payment, $payment_mode[0]);
        ...
        $module_entry_id = $max_booking_id;
        $transaction_id = "";
        $payment_amount = $balance_amount;
        $payment_date = $booking_date;
        $payment_particular = get_sales_particular(get_package_booking_id($max_booking_id), $booking_date, $balance_amount, $customer_id);
...

{
  ...
  //**Saves Train Information    
...

      {
        ...
        $sq = mysql_query(" insert into package_plane_master (plane_id, booking_id, date,from_city, from_location, to_city,to_location, company, amount, seats, arraval_time ) values ('$max_plane_id', '$max_booking_id', '$plane_travel_date[$i]', '$from_city_id_arr[$i]',  '$plane_from_location[$i]', '$to_city_id_arr[$i]', '$plane_to_location[$i]', '$plane_company[$i]', '$plane_amount[$i]', '$plane_seats[$i]', '$arraval_arr[$i]') ");
...
//Transport Save
function package_tour_tranpsort_information_save($max_booking_id, $transp_vehicle_arr, $transp_start_date, $trans_end_date){
    for($i=0; $i<sizeof($transp_vehicle_arr); $i++){
        ...
        $trans_end_date[$i] = mysql_real_escape_string($trans_end_date[$i]);

        $transp_start_date[$i] = date("Y-m-d", strtotime($transp_start_date[$i]));
        $trans_end_date[$i] = date("Y-m-d", strtotime($trans_end_date[$i]));
      $q1 = "insert into package_tour_transport_master (entry_id, booking_id, transport_bus_id, transport_from_date, transport_to_date) values ('$max_entry_id', '$max_booking_id', '$transp_vehicle_arr[$i]', '$transp_start_date[$i]', '$trans_end_date[$i]')";
        $sq = mysql_query($q1);
...
//Excursion Save
function package_tour_exc_information_save($max_booking_id, $exc_city_arr, $exc_name_arr){
  for($i=0; $i<sizeof($exc_city_arr); $i++){
      ...
      $sq = mysql_query("insert into package_tour_excursion_master (entry_id, booking_id, city_id, exc_id) values ('$max_entry_id', '$max_booking_id', '$exc_city_arr[$i]', '$exc_name_arr[$i]')");

      if(!$sq){
        $GLOBALS['flag'] = false;
        echo "Error at row".($i+1)." for Excursion information.";
      }
  }
}
...
///////////////////////////////////***** Package Tour payment master save start *********/////////////////////////////////////////////////////////////
 ...
 function package_tour_payment_master_save($customer_id,$max_booking_id, $payment_date, $payment_mode, $payment_amount, $bank_name, $transaction_id, $payment_for, $p_travel_type, $bank_id_arr, $branch_admin_id , $emp_id,$total_travel_expense,$actual_tour_cost,$booking_date)

 {
...

    {
       ...
       $clearance_status = ($payment_mode[$i]=="Cheque") ? "Pending" : "";   
     ...
     $sq= mysql_query(" insert into package_payment_master (payment_id, booking_id, financial_year_id, branch_admin_id , emp_id ,date, payment_mode, amount, bank_name, transaction_id, payment_for, travel_type, bank_id, clearance_status, advance_status ) values ('$max_payment_id', '$max_booking_id', '$financial_year_id', '$branch_admin_id' , '$emp_id', '$payment_date[$i]', '$payment_mode[$i]', '$payment_amount[$i]', '$bank_name[$i]', '$transaction_id[$i]', '$payment_for[$i]', '$p_travel_type[$i]', '$bank_id_arr[$i]', '$clearance_status','true') ");
     ...
     if(!$sq)
...

      {
        ...
        $booking_save_transaction->payment_finance_save($max_booking_id, $max_payment_id, $branch_admin_id, $payment_date[$i], $payment_mode[$i], $payment_amount[$i], $transaction_id[$i], $bank_id_arr[$i], $payment_date[$i], $clearance_status);



        //Bank and Cash Book Save

        $booking_save_transaction->bank_cash_book_save($max_booking_id, $max_payment_id, $payment_date[$i], $payment_mode[$i], $payment_amount[$i], $transaction_id[$i], $payment_date[$i], $bank_name[$i], $bank_id_arr[$i], $branch_admin_id);
...
function send_mail_to_traveler($booking_id, $tour_name, $tour_from_date, $tour_to_date, $customer_id, $email_id)
              ...
              <tr>
                <td colspan="2">
                  <table style="width:100%"> 
                    ...
                    <tr>
                      <td style="padding-top:15px"><em style="'.$mail_em_style.'"><strong style="'.$mail_strong_style.'">Booking ID:</strong><span style="color:#fff;"> '.get_package_booking_id($booking_id,$year).'</span></em></td>
                    </tr>
                    <tr>
                      <td><em style="'.$mail_em_style.'"><strong style="'.$mail_strong_style.'">Tour Name:</strong><span style="color:#fff;"> '.$tour_name.'</span></em></td>
                    </tr>
                    <tr>
                      <td><em style="'.$mail_em_style.'"><strong style="'.$mail_strong_style.'">Journey Date:</strong><span style="color:#fff;"> '.$from_date.' to '.$to_date.'</span></em></td>
                    </tr>
                  </table>
                </td>
              </tr>
...
$model->app_email_send('14',$email_id, $content, $subject);
$model->app_email_send('14',$backoffice_email_id, $content, $subject);
...
//////////////////////////////////**Booking successfull sms send to traveler start**/////////////////////////////////////
...

 {
    ...
    $message = "Dear ".$fname." ".$lname.", your booking for ".$tour_name." has been confirmed. Voucher details sent on your Mail ID within 3 working days.";
