...
public function quotation_master_save()
{
	...
	$extra_bedc_arr = $_POST['extra_bedc_arr'];
	...
	for($i=0; $i<sizeof($package_id_arr); $i++){
		...
		$sq_quotation = mysql_query("insert into package_tour_quotation_master ( quotation_id,enquiry_id, branch_admin_id,financial_year_id, tour_name, from_date, to_date, total_days, customer_name, email_id,mobile_no, total_adult, total_children, total_infant, total_passangers, children_without_bed, children_with_bed, quotation_date, booking_type, train_cost, flight_cost, cruise_cost, visa_cost, guide_cost,misc_cost, price_str_url, package_id, created_at, login_id,emp_id,inclusions,exclusions,pckg_daywise_url,costing_type ) values ( '$quotation_id','$enquiry_id', '$branch_admin_id','$financial_year_id', '$tour_name', '$from_date', '$to_date', '$total_days', '$customer_name', '$email_id','$mobile_no', '$total_adult', '$total_children', '$total_infant', '$total_passangers', '$children_without_bed', '$children_with_bed', '$quotation_date', '$booking_type', '$train_cost','$flight_cost','$cruise_cost','$visa_cost','$guide_cost','$misc_cost','$price_str_url','$package_id_arr[$i]', '$created_at', '$login_id', '$emp_id','$incl','$excl','$pckg_daywise_url','$costing_type')");
    }
	if($sq_quotation){
		////////////Enquiry Save///////////
		if($enquiry_id == 0){
		...
		$this->hotel_entries_save($quotation_id_arr, $city_name_arr, $hotel_name_arr,$hotel_type_arr, $hotel_stay_days_arr, $package_name_arr,$total_rooms_arr,$hotel_cost_arr,$extra_bed_cost_arr,$extra_bed_arr);
		$this->tranport_entries_save($quotation_id_arr,$vehicle_name_arr, $start_date_arr, $end_date_arr, $package_name_arr1,$transport_cost_arr1);	
		$this->costing_entries_save($quotation_id_arr,$tour_cost_arr, $markup_cost_arr,$markup_subtotal_arr, $taxation_id_arr,$service_tax_arr,$service_tax_subtotal_arr,$total_tour_cost_arr, $package_name_arr2,$transport_cost_arr,$excursion_cost_arr,$adult_cost_arr,$child_cost_arr,$infant_cost_arr,$child_with_arr,$child_without_arr,$extra_bedc_arr);
		$this->excursion_entries_save($quotation_id_arr,$city_name_arr_e, $excursion_name_arr, $excursion_amt_arr);		
...
public function hotel_entries_save($quotation_id_arr, $city_name_arr, $hotel_name_arr,$hotel_type_arr, $hotel_stay_days_arr, $package_name_arr,$total_rooms_arr,$hotel_cost_arr,$extra_bed_cost_arr,$extra_bed_arr)
{
	$j=0;
...
public function tranport_entries_save($quotation_id_arr,$vehicle_name_arr, $start_date_arr, $end_date_arr, $package_name_arr1,$transport_cost_arr1)
{
	...
	for($i=0; $i<sizeof($vehicle_name_arr); $i++){
		...
		if($i == 0){
			$sq_plane = mysql_query("insert into package_tour_quotation_transport_entries2 ( id, quotation_id, vehicle_name, start_date, end_date,package_id,transport_cost ) values ( '$id', '$quotation_id_arr[$j]', '$vehicle_name_arr[$i]', '$start_date_arr[$i]', '$end_date_arr[$i]','$package_id','$transport_cost_arr1[$i]' )");
		}
		else{
...
public function excursion_entries_save($quotation_id_arr,$city_name_arr_e, $excursion_name_arr, $excursion_amt_arr)
{
	for($i=0; $i<sizeof($quotation_id_arr); $i++){
		...
		$sq_plane = mysql_query("insert into package_tour_quotation_excursion_entries ( id, quotation_id, city_name, excursion_name, excursion_amount ) values ( '$id', '$quotation_id_arr[$i]', '$city_name_arr_e[$j]','$excursion_name_arr[$j]', '$excursion_amt_arr[$j]')");
		if(!$sq_plane){
...
public function costing_entries_save($quotation_id_arr,$tour_cost_arr, $markup_cost_arr,$markup_subtotal_arr, $taxation_id_arr,$service_tax_arr,$service_tax_subtotal_arr,$total_tour_cost_arr, $package_name_arr2,$transport_cost_arr,$excursion_cost_arr,$adult_cost_arr,$child_cost_arr,$infant_cost_arr,$child_with_arr,$child_without_arr,$extra_bedc_arr)
{
	for($i=0; $i<sizeof($tour_cost_arr); $i++){
		...
		$sq_plane = mysql_query("insert into package_tour_quotation_costing_entries ( id, quotation_id, tour_cost, markup_cost,markup_subtotal, excursion_cost,  taxation_id, service_tax, service_tax_subtotal, total_tour_cost, package_id, transport_cost,adult_cost,child_cost,infant_cost,child_with,child_without,extra_bedc ) values ( '$id', '$quotation_id_arr[$i]', '$tour_cost_arr[$i]', '$markup_cost_arr[$i]','$markup_subtotal_arr[$i]','$excursion_cost_arr[$i]', '$taxation_id_arr[$i]', '$service_tax_arr[$i]', '$service_tax_subtotal_arr[$i]', '$total_tour_cost_arr[$i]','$package_id','$transport_cost_arr[$i]','$adult_cost_arr[$i]','$child_cost_arr[$i]','$infant_cost_arr[$i]','$child_with_arr[$i]','$child_without_arr[$i]','$extra_bedc_arr[$i]' )");
