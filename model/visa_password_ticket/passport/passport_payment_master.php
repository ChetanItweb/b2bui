...
public function payment_email_notification_send($passport_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   $sq_customer_info = mysql_fetch_assoc(mysql_query("select email_id from customer_master where customer_id='$sq_passport_info[customer_id]'"));
   $email_id = $sq_customer_info['email_id'];

   $sq_total_amount = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from passport_payment_master where passport_id='$passport_id'"));
   ...
   global $model;
   $model->generic_payment_mail('52',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $email_id,$subject);
}
...
public function payment_sms_notification_send($passport_id, $payment_amount, $payment_mode)
{
	...
	$message = "Acknowledge your payment of Rs. ".$payment_amount.", ".$payment_mode." which we received for Passport installment.";
    global $model;
