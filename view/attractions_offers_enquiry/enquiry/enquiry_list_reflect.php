...
$newIds=array();
$sq_ids=mysql_query("SELECT enquiry_id, MAX(entry_id) AS entry_id , followup_status FROM enquiry_master_entries GROUP BY enquiry_id");

while($row1=mysql_fetch_assoc($sq_ids)){
	$newIds[]=$row1['entry_id'];
}
$ids=implode(',', $newIds);
...
$('#limit').html('<?php echo $limit;?>');
$('#start').html('<?php echo $start;?>');
