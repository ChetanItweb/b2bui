...
<tr>
    ...
    <td><input type="text" id="cruise_departure_date<?= $count_t ?>t" name="cruise_departure_date<?= $count_t ?>t" placeholder="Departure Date & Time" title="Departure Date & Time" class="app_datetimepicker" value="<?= get_datetime_user($row_cruise_details['dept_datetime']) ?>" onchange="get_to_datetime(this.id,'cruise_arrival_date<?= $count_t ?>t')" style="width:136px"></td>
    <td><input type="text" id="cruise_arrival_date<?= $count_t ?>t" name="cruise_arrival_date<?= $count_t ?>t" placeholder="Arrival Date & Time" title="Arrival Date & Time" class="app_datetimepicker" value="<?= get_datetime_user($row_cruise_details['arrival_datetime']) ?>" style="width:136px"></td>
...
<?php if($row_cruise_details['sharing']!='') { ?><option value="<?= ($row_cruise_details['sharing']) ?>"><?= ($row_cruise_details['sharing']) ?></option><?php }  ?>
    ...
    <td><input type="text" id="txt_cruise_amount1<?= $count_t ?>t" name="txt_cruise_amount1<?= $count_t ?>t" placeholder="*Amount" title="Amount" onchange="validate_balance(this.id);" value="<?= $row_cruise_details['amount'] ?>" onkeyup=" calculate_cruise_expense('tbl_dynamic_cruise_package_booking');"  style="width:120px"/></td>
