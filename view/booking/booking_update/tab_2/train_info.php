...
<tr>
	...
	<td><input type="text" id="<?php echo 'txt_train_date'.$count_t.'t' ?>" name="<?php echo 'txt_train_date'.$count_t.'t' ?>" placeholder="Departure Date & Time" title="Departure Date & Time" value="<?php echo date("d-m-Y H:i:s", strtotime($row_train_details['date'])) ?>" style="width:136px"/></td>
	...
	<td><input type="text" id="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" name="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" placeholder="Total Seats" title="Total Seats" maxlength="3" onchange="validate_balance(this.id);"  onkeyup="calculate_service_charge_seat(this.id);" value="<?php echo $row_train_details['seats'] ?>" style="width:100px"/></td>

	<td><input type="text" id="<?php echo 'txt_train_amount'.$count_t.'t' ?>" name="<?php echo 'txt_train_amount'.$count_t.'t' ?>" title="Amount" placeholder="Amount" onchange="validate_balance(this.id);" onkeyup=" calculate_train_expense('tbl_train_travel_details_dynamic_row');" value="<?php echo $row_train_details['amount'] ?>"  style="width:120px"/></td>
    ...
    <div class="row">
        ...
        <div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10_xs">
            ...
            <select name="train_taxation_id" id="train_taxation_id" title="Tax" onchange="generic_tax_reflect(this.id, 'train_service_tax', 'calculate_total_train_expense');">
            	<?php if($tourwise_details['train_taxation_id'] != '0'){            	
            		$sq_taxation = mysql_fetch_assoc(mysql_query("select * from taxation_master where taxation_id='$tourwise_details[train_taxation_id]'"));
