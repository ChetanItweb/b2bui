...
$('#frm_tab_3').validate({
    ...
    submitHandler:function(form){
          ...
          {       
              if(payment_mode1!="Cash" && payment_mode1!="Credit Note" && payment_mode1!="Credit Card" && payment_mode1!="Online" && payment_amount1 != "0")
              { 
                if(bank_name1 == "") {  error_msg_alert("Enter tour payment bank name."); return false; }  
                if(transaction_id1 == "") { error_msg_alert("Enter tour payment transaction ID."); return false; }  
                if(bank_id1 == "") { error_msg_alert("Select creditor bank name."); return false; }  
              }
          }

          if(traveling_payment==true)
          {       
              if(payment_mode2!="Cash" && payment_mode2!="Credit Note" && payment_mode2!="Credit Card" && payment_mode2!="Online" && payment_amount2 != "0"){ 
                if(bank_name2 == ""){  error_msg_alert("Enter travel payment bank name."); return false; }  
                if(transaction_id2 == ""){ error_msg_alert("Enter travel payment transaction ID."); return false; }  
                if(bank_id2 == "") { error_msg_alert("Select creditor bank name."); return false; }  
              }
              if(travel_of_type2 == ""){ error_msg_alert("Enter travel payment for."); return false; }  
          }

          var total_payment = parseFloat(payment_amount1) + parseFloat(payment_amount2);
          if(credit_amount != ''){ 
            if(parseFloat(total_payment) > parseFloat(credit_amount)) { error_msg_alert('Low Credit note balance'); return false; }
          }
...
function save_booking_details()
{
  ...
  $.post(base_url+'view/load_data/finance_date_validation.php', { check_date: check_date1 }, function(data){
    if(data !== 'valid'){
      ...
      $.post(base_url+'view/load_data/finance_date_validation.php', { check_date: test_payment_date2 }, function(data){
      if(traveling_payment == true){
        if(data !== 'valid'){
