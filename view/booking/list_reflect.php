	...
	<tbody>
		...
		while($row_booking = mysql_fetch_assoc($sq_booking)){
			...
			<tr class="<?= $bg ?>">
				...
				<td><?= ($sq_customer['type'] == 'Corporate') ? $sq_customer['company_name'] : $sq_customer['first_name'].' '.$sq_customer['last_name'] ?></td>
				...
				<td>
					...
					<button class="btn btn-info btn-sm" onclick="display_modal(<?php echo $row_booking['id']; ?>)" title="View Detail"><i class="fa fa-eye"></i></button>

				</td>

				<td>

					<form action="booking_update/booking_update.php" id="frm_booking_<?= $count ?>" method="POST">

						<input type="hidden" id="booking_id" name="booking_id" value="<?= $row_booking['id'] ?>">
