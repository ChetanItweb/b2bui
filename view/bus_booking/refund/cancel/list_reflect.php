...
$sq_payment_info = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from bus_booking_payment_master where booking_id='$booking_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
if($booking_id==""){ exit; }
?>
...
<div class="row">	
	...
	<div class="col-md-6 col-sm-12 col-xs-12">
		<div class="widget_parent-bg-img bg-img-red">
			<?php
	            begin_widget();
				$title_arr = array("Basic Cost", "Service Charge","Tax", "Net Total");
				$content_arr = array( $sq_booking['basic_cost'] , $sq_booking['service_charge'], $sq_booking['service_tax_subtotal'],$sq_booking['net_total']);
				$percent = ($sq_payment_info['sum']/$sq_booking['net_total'])*100;
	...
	<div class="col-md-6 col-md-offset-0 col-sm-8 col-sm-offset-2 col-xs-12"> <div class="table-responsive">
		...
		<table class="table table-hover table-bordered mg_bt_0" style="margin: 0 !important;">
			...
			<?php
			$count = 0;
			$sq_entries = mysql_query("select * from bus_booking_entries where booking_id='$booking_id'");
			while($row_entry = mysql_fetch_assoc($sq_entries)){
		...
		<div class="panel panel-default panel-body text-center">
			<button class="btn btn-danger btn-sm ico_left" id="cancel_booking" onclick="cancel_booking()"><i class="fa fa-times"></i>&nbsp;&nbsp;Cancel</button>
		</div>
...
<?php 
$sq_cancel_count = mysql_num_rows(mysql_query("select * from bus_booking_entries where booking_id='$booking_id' and status='Cancel'"));
...
<form id="frm_refund" class="mg_bt_150">
	...
	<div class="row text-center">
		<div class="col-md-3 col-md-offset-3 col-sm-6 col-xs-12 mg_bt_10_xs">
			<input type="text" name="cancel_amount" id="cancel_amount" class="text-right" placeholder="*Cancel Amount" title="Cancel Amount" onchange="calculate_total_refund();validate_balance(this.id);" value="<?= $sq_booking['cancel_amount'] ?>">
		</div>
		<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
			<input type="text" name="total_refund_amount" id="total_refund_amount" class="amount_feild_highlight text-right" placeholder="Total Refund" title="Total Refund" readonly value="<?= $sq_booking['refund_net_total'] ?>">
		</div>
	</div>
	<?php if($sq_booking['cancel_amount'] == "0.00"){ ?>
	<div class="row mg_tp_20">
...
$('#frm_refund').validate({
	rules:{
			refund_basic_cost : { required : true, number : true },
	},
	submitHandler:function(){
	  ...
	  if(parseFloat(cancel_amount) > parseFloat(total_sale)) { error_msg_alert("Cancel amount can not be greater than Sale amount"); return false; }
		...
		$.ajax({
			type:'post',
			url: base_url()+'controller/bus_booking/refund_estimate/refund_estimate.php',
