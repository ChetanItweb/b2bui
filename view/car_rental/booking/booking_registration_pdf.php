...
$pdf->Row(array('Travelling Date', date('d-m-Y H:i:s', strtotime($sq_booking['traveling_date'])), 'Days Of Travel', $sq_booking['days_of_traveling']));

$pdf->SetWidths(array(30,160));
$pdf->Row(array('Passenger', $sq_booking['pass_name']));
$pdf->Row(array('Address', $sq_customer['address']));
$pdf->Row(array('Places To Visit', $sq_booking['places_to_visit']));
...
while($row_vehicle = mysql_fetch_assoc($sq_vehicle_entries)){
	...
	$pdf->Row(array($count, $sq_vehicle['vehicle_name'], $sq_vehicle['vehicle_no'], $sq_vehicle['vehicle_driver_name'], $sq_vehicle['vehicle_mobile_no'], $sq_vehicle['vehicle_type']));
