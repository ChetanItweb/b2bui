...
<script>
$('#dest_name_s,#city_name,#vehicle_name1').select2();
...
$(function(){
  ...
  $('#frm_package_master_save').validate({
    ...
    submitHandler:function(form){
        ...
        // for(var i=0; i<rowCount; i++){
        //     var row = table.rows[i];
        //     var flag = validate_spattration(row.cells[0].childNodes[0].id);
        //     alert(flag);
        //     if(!flag){
        //       return false;
        //     }
...

{
  ...
  for(var i=0; i<rowCount; i++){
      ...
      validate_spattration(row.cells[0].childNodes[0].id);
      validate_dayprogram(row.cells[1].childNodes[0].id);
      validate_onstay(row.cells[2].childNodes[0].id);
          ...
          if(row.cells[0].childNodes[0].checked)
           ...
           {  
              ...
              if(row.cells[5].childNodes[0].value==""){
                    ...
                    validate_message += "Enter Total Days in row-"+(i+1)+"<br>";
