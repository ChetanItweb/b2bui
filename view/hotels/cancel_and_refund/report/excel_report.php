...
if(($booking_id!="")){
    $sq_entry_date = mysql_fetch_assoc(mysql_query("select * from hotel_booking_master where booking_id='$booking_id'"));
    $date = $sq_entry_date['created_at'];
    $yr = explode("-", $date);
    $year =$yr[0];
    $invoice_id = get_hotel_booking_id($booking_id,$year);
}
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Hotel Booking Refund')
...
header('Content-Disposition: attachment;filename="Hotel Booking Refund('.date('d-m-Y H:i:s').').xls"');
