$(function(){
	$('#frm_tab_4').validate({
		rules:{
				...
				//txt_special_request : { required :true },
				txt_booking_date : { required :true },
				tour_taxation_id : { required : true},
				txt_payment_date1: { required: function(){ if($("#chk_pay_for_tour").is(':checked') == 1){ return true; } else { return false; } } },
	            cmb_payment_mode1: { required: function(){ if($("#chk_pay_for_tour").is(':checked') == 1){ return true; } else { return false; } } },
	            txt_amount1: { required: function(){ if($("#chk_pay_for_tour").is(':checked') == 1){ return true; } else { return false; } } },

	            txt_payment_date2: { required: function(){ if($("#chk_pay_for_traveling").is(':checked') == 1){ return true; } else { return false; } } },
	            cmb_payment_mode2: { required: function(){ if($("#chk_pay_for_traveling").is(':checked') == 1){ return true; } else { return false; } } },
	            txt_amount2: { required: function(){ if($("#chk_pay_for_traveling").is(':checked') == 1){ return true; } else { return false; } } },
	            cmb_travel_of_type2: { required: function(){ if($("#chk_pay_for_traveling").is(':checked') == 1){ return true; } else { return false; } } },
		},
		submitHandler:function(form){
			...
			if(financial_year_id == ''){
			  ...
			  for(var i=0; i<rowCount; i++)
			  {
			    ...
			    {              
		          ...
		          var from_location1 = row.cells[4].childNodes[0].value;  
			  ...
			  for(var i=0; i<rowCount; i++){
			    ...
			    if(row.cells[0].childNodes[0].checked){
			        ...
			        var start_date = row.cells[3].childNodes[0].value;
							...
							trans_end_date.push(end_date);
			    }
				}
				
				//**Excursion travel details starts here 
		    ...
		    var exc_name_arr = new Array();
			  ...
			  for(var i=0; i<rowCount; i++){
			    ...
			    if(row.cells[0].childNodes[0].checked){
			        var vehicle = row.cells[2].childNodes[0].value;
							...
							exc_city_arr.push(vehicle);
							exc_name_arr.push(start_date);
			    }
				}
			  ...
			  var total_tour_cost = $("#txt_tour_cost").val();
			 ...
			 if(parseFloat(tour_taxation_id) == "0"){ error_msg_alert("Please select Tax Percentage"); return false; }
			  ...
			  var tour_cost_total = $('#txt_actual_tour_cost1').val();
			  ...
			  {
			  	if(payment_mode1!="Cash" && payment_mode1!="Credit Note" && payment_mode1!="Credit Card"&& payment_mode1!="NA" && payment_mode1!="Online" && payment_amount1 != '0' ){
			  		if(bank_name1==""){
			  ...
			  {
			  	if(payment_mode2!="Cash" && payment_mode2!="Credit Note" && payment_mode2!="Credit Card" && payment_mode2!="NA" && payment_mode2!="Online" && payment_amount2 != '0' ){
			  		if(bank_name2==""){
			  			error_msg_alert("Enter bank name");
			  			return false;
			  		}
			  ...
			  var total_payment_amount = parseFloat(payment_amount1) + parseFloat(payment_amount2);
		        if(credit_amount != ''){ 
		        	if(parseFloat(total_payment_amount) > parseFloat(credit_amount)) { error_msg_alert('Low Credit note balance'); return false; }
		        }
			  ...
			  booking_date = $(booking_date1).val();
				...
				$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: check_date1 }, function(data){
					if(data !== 'valid'){
						...
						$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: test_payment_date }, function(data){
						if(pay_for_tour == true)
							if(data !== 'valid'){
							...
							$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: test_payment_date1 }, function(data){
							if(pay_for_traveling == true){
								if(data !== 'valid'){
							...
							if(pay_for_traveling == true || pay_for_traveling == false){
								...
								callback: function(result){
									if(result=="yes"){
											...
											$.post( 
																	base_url+"controller/package_tour/booking/package_tour_booking_master_save_c.php",
																	{ quotation_id : quotation_id,dest_id: dest_id,package1:package1, emp_id : emp_id, customer_id : customer_id, unique_timestamp : unique_timestamp, tour_name : tour_name,package_id : package_id, tour_type : tour_type, tour_from_date : tour_from_date, tour_to_date : tour_to_date, total_tour_days : total_tour_days, taxation_type : taxation_type, 'm_honorific[]' : m_honorific, 'm_first_name[]' : m_first_name, 'm_middle_name[]' : m_middle_name, 'm_last_name[]' : m_last_name, 'm_gender[]' : m_gender, 'm_birth_date[]' : m_birth_date, 'm_age[]' : m_age, 'm_adolescence[]' : m_adolescence, m_passport_no : m_passport_no, m_passport_issue_date : m_passport_issue_date, m_passport_expiry_date : m_passport_expiry_date, contact_person_name : contact_person_name, email_id : email_id, mobile_no : mobile_no, address : address, country_name : country_name, city : city, state : state , train_expense : train_expense, train_service_charge : train_service_charge, train_taxation_id : train_taxation_id, train_service_tax : train_service_tax, train_service_tax_subtotal : train_service_tax_subtotal, total_train_expense : total_train_expense, plane_expense : plane_expense, plane_service_charge : plane_service_charge, plane_taxation_id : plane_taxation_id, plane_service_tax : plane_service_tax, plane_service_tax_subtotal : plane_service_tax_subtotal, total_plane_expense : total_plane_expense, total_travel_expense : total_travel_expense, train_ticket_path : train_ticket_path, plane_ticket_path : plane_ticket_path, 'train_travel_date[]' : train_travel_date, 'train_from_location[]' : train_from_location, 'train_to_location[]' : train_to_location, 'train_train_no[]' : train_train_no, 'train_travel_class[]' : train_travel_class, 'train_travel_priority[]' : train_travel_priority, 'train_amount[]' : train_amount, 'train_seats[]' : train_seats, 'plane_travel_date[]' : plane_travel_date, 'plane_from_location[]' : plane_from_location, 'plane_to_location[]' : plane_to_location, 'plane_amount[]' : plane_amount, 'plane_seats[]' : plane_seats, 'plane_company[]' : plane_company, from_city_id_arr : from_city_id_arr,  to_city_id_arr : to_city_id_arr,  'city_id[]' : city_id, 'hotel_id[]' : hotel_id, 'hotel_from_date[]' : hotel_from_date, 'hotel_to_date[]' : hotel_to_date, 'hotel_rooms[]' : hotel_rooms, 'hotel_catagory[]' : hotel_catagory, room_type : room_type, 'meal_plan[]' : meal_plan,transp_vehicle_arr:transp_vehicle_arr,transp_start_date:transp_start_date,trans_end_date:trans_end_date,exc_city_arr:exc_city_arr,exc_name_arr:exc_name_arr, confirmation_no : confirmation_no, visa_country_name : visa_country_name, visa_amount : visa_amount, visa_service_charge : visa_service_charge, visa_taxation_id : visa_taxation_id, visa_service_tax : visa_service_tax, visa_service_tax_subtotal : visa_service_tax_subtotal, visa_total_amount : visa_total_amount, insuarance_company_name : insuarance_company_name, insuarance_amount : insuarance_amount, insuarance_service_charge : insuarance_service_charge, insuarance_taxation_id : insuarance_taxation_id, insuarance_service_tax : insuarance_service_tax, insuarance_service_tax_subtotal : insuarance_service_tax_subtotal, insuarance_total_amount : insuarance_total_amount, total_hotel_expense : total_hotel_expense, total_tour_cost : total_tour_cost, subtotal : subtotal, tour_taxation_id : tour_taxation_id, tour_service_tax : tour_service_tax, tour_service_tax_subtotal : tour_service_tax_subtotal, currency_code : currency_code, rue_cost : rue_cost, subtotal_with_rue : subtotal_with_rue, tour_cost_total : tour_cost_total, actual_tour_cost : actual_tour_cost, 'payment_date[]' : payment_date, 'payment_mode[]' : payment_mode, 'payment_amount[]' : payment_amount, 'bank_name[]' : bank_name, 'transaction_id[]' : transaction_id, 'payment_for[]' : payment_for, 'p_travel_type[]' : p_travel_type, bank_id_arr : bank_id_arr, special_request : special_request, due_date : due_date, booking_date : booking_date ,'arraval_arr[]' : arraval_arr, cruise_dept_date_arr : cruise_dept_date_arr,cruise_arrival_date_arr : cruise_arrival_date_arr,route_arr : route_arr,cabin_arr : cabin_arr,sharing_arr : sharing_arr,cruise_seats_arr : cruise_seats_arr, cruise_amount_arr : cruise_amount_arr,cruise_ticket_path : cruise_ticket_path, cruise_expense : cruise_expense, cruise_service_charge : cruise_service_charge, cruise_taxation_id : cruise_taxation_id, cruise_service_tax : cruise_service_tax, cruise_service_tax_subtotal : cruise_service_tax_subtotal, total_cruise_expense : total_cruise_expense, branch_admin_id : branch_admin_id,financial_year_id :financial_year_id,incl:incl,excl:excl,special_attraction_arr:special_attraction_arr,day_program_arr:day_program_arr,stay_arr:stay_arr,meal_plan_arr:meal_plan_arr},
																	function(data) {      
																					console.log(data);
