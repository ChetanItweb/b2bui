$('#txt_package_from_date,#txt_package_to_date,#txt_m_passport_issue_date1,#txt_m_passport_expiry_date1').datetimepicker({ 
...
$('#customer_id_p,#quotation_id, #country_name, #txt_m_state').select2();
...
function customer_info_load(div_id,offset='')
{
  ...
  $.ajax({
    ...
    success:function(result){
      ...
      $('#txt_contact_person_name'+offset).val(result.first_name+' '+result.middle_name+' '+result.last_name);
...
$(function(){
  $('#frm_tab_1').validate({
    ...
    submitHandler:function(form){
        ...
        if(customer_id == ''||customer_id == 0){ error_msg_alert("Select Customer!");return false;}
...
function package_tour_booking_tab1_validate()
{
  ...
  for(var i=0; i<rowCount; i++)
  {
    ...
    if(row.cells[0].childNodes[0].checked){
      ...
      if(!row.cells[7].childNodes[0].value.match(/^([0-9]{2})\-([0-9]{2})\-([0-9]{4})$/) && !row.cells[7].childNodes[0].value.match(/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/)){
      ...
      {
         if(!row.cells[11].childNodes[0].value.match(/^([0-9]{2})\-([0-9]{2})\-([0-9]{4})$/) && !row.cells[11].childNodes[0].value.match(/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/)){
      ...
      {
         if(!row.cells[12].childNodes[0].value.match(/^([0-9]{2})\-([0-9]{2})\-([0-9]{4})$/) && !row.cells[12].childNodes[0].value.match(/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/)){
  ...
  if(quotation_id != 0){
    ...
    $.ajax({
      ...
      success:function(result){
        ...
        //alert(response.total_passangers);
        ...
        $('#txt_tour_cost').val(response.markup_cost);
        $('#subtotal').val(subtotal);
        var tax = response.tax_type+'-'+response.tax_in_percentage;
        var tax_id = response.taxation_id;
        taxes_reflect(tax_id,tax);
        $('#txt_tour_service_tax').val(response.service_tax);
        $('#tour_service_tax_subtotal').val(response.service_tax_subtotal);
        $('#txt_actual_tour_cost1').val(response.total_tour_cost);
        $('#txt_actual_tour_cost').val(response.total_tour_cost);
        ...
        for(var i=0; i<exc_info_arr.length; i++){
            var row = table.rows[i];
            row.cells[2].childNodes[0].value = exc_info_arr[i]['city_id'];
            $(row.cells[3].childNodes[0]).html('<option value="'+exc_info_arr[i]['exc_id']+'">'+exc_info_arr[i]['exc_name']+'</option>');
        }
        ...
        for(var i=0; i<flight_info_arr.length; i++){
              ...
              row.cells[3].childNodes[0].value = flight_info_arr[i]['from_city_id'];
              $(row.cells[4].childNodes[0]).html('<option value="'+flight_info_arr[i]['from_location']+'">'+flight_info_arr[i]['from_location']+'</option>');
              
              row.cells[5].childNodes[0].value = flight_info_arr[i]['to_city_id'];
              
              $(row.cells[6].childNodes[0]).html('<option value="'+flight_info_arr[i]['to_location']+'">'+flight_info_arr[i]['to_location']+'</option>');
              
              
              row.cells[7].childNodes[0].value = flight_info_arr[i]['airline_name'];
              row.cells[10].childNodes[0].value = flight_info_arr[i]['arrival_date'];
        ...
        for(var i=0; i<hotel_info_arr.length; i++){
              ...
              row.cells[2].childNodes[0].value = hotel_info_arr[i]['city_id'];
              $(row.cells[3].childNodes[0]).html('<option value="'+hotel_info_arr[i]['hotel_id1']+'">'+hotel_info_arr[i]['hotel_name1']+'</option>');
              row.cells[4].childNodes[0].value = response.from_date;
              row.cells[5].childNodes[0].value = response.to_date;
  ...
  $.ajax({
    ...
    success:function(result){
      ...
      for(var i=0; i<hotel_info_arr.length; i++){
            var row = table.rows[i];
            row.cells[2].childNodes[0].value = hotel_info_arr[i]['city_id'];
            $(row.cells[3].childNodes[0]).html('<option value="'+hotel_info_arr[i]['hotel_id1']+'">'+hotel_info_arr[i]['hotel_name']+'</option>');
      }
    ...
    for(var i=0; i<transport_info_arr.length; i++){
        var row = table.rows[i];
        row.cells[2].childNodes[0].value = transport_info_arr[i]['bus_id'];
    }
