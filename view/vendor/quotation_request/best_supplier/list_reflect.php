...
<table class="table table-bordered table-hover bg_white" id="tbl_req_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_req = mysql_fetch_assoc($sq_req)){
			$sq_request = mysql_fetch_assoc(mysql_query("select city_id,enquiry_id,quotation_for from vendor_request_master where request_id='$row_req[request_id]' "));
