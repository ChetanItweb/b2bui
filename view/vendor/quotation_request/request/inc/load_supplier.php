...
while($sq_city=mysql_fetch_assoc($city)){
    $getCity=mysql_fetch_assoc(mysql_query("select city_name from city_master where city_id=$sq_city[city_id]"));
?>
    <optgroup label="<?= $getCity['city_name'];?>">
    <?php
...
$city=mysql_query("select distinct(city_id) from dmc_master");
	...
	?>
        <optgroup label="<?= $getCity['city_name'];?>" value="<?= $getCity['city_id'];?>">
        <?php
