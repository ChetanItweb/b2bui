...
if($passport_id != ''){
    $sq_entry_date = mysql_fetch_assoc(mysql_query("select * from passport_master where passport_id='$passport_id'"));
    $date = $sq_entry_date['created_at'];
    $yr = explode("-", $date);
    $year =$yr[0];
    $invoice_id = get_passport_booking_id($passport_id,$year);
}
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Passport Booking Refund')
...
while($row_refund = mysql_fetch_assoc($sq_refund)){
      ...
      if($row_refund['clearance_status']=="Pending"){ 
        ...
        ->setCellValue('C'.$row_count, get_passport_booking_id($row_refund['passport_id'],$year))
...
header('Content-Disposition: attachment;filename="Passport Booking Refund('.date('d-m-Y H:i:s').').xls"');
