			...
			<div class="row mg_bt_10">
				<div class="col-xs-12">
				  	<select name="entry_id" id="entry_id" multiple>
				  		<?php 
				  		$sq_train_ticket_entries = mysql_query("select * from train_ticket_master_entries where train_ticket_id='$train_ticket_id' and status='Cancel'");
	...
	<div class="panel panel-default panel-body no-pad"> 
		...
		<table class="table table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				$sq_train_ticket_refund = mysql_query($query);
				while($row_train_ticket_refund = mysql_fetch_assoc($sq_train_ticket_refund)){
			...
			<tfoot>
				<?php 
				$sq_refund_pen_amount = mysql_fetch_assoc(mysql_query("select sum(refund_amount) as sum from train_ticket_refund_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]' and clearance_status='Pending'"));
				$sq_refund_can_amount = mysql_fetch_assoc(mysql_query("select sum(refund_amount) as sum from train_ticket_refund_master where train_ticket_id='$sq_train_ticket_info[train_ticket_id]' and clearance_status='Cancelled'"));
				...
				<tr class="active">
					...
					<th class="text-right danger" colspan="2">Cancelled : <?=  number_format($Cancelled_amt,2); ?></th>
					...
					<input type="hidden" name="remaining_val" id="remaining_val" value="<?php echo $refund_amount-$total_refund;?>">
				</tr>
			</tfoot>
		</table>
		</div> 
...
$(function(){
  $('#frm_refund_save').validate({
      ...
      submitHandler:function(form){
             ...
             // alert($('#refund_amount').val());
              if( Number(refund_amount) > Number(remaining_val))
              ...
              $('#vi_confirm_box').vi_confirm_box({
                ...
                callback: function(data1){
                    if(data1=="yes"){
                        ...
                        $.ajax({
                          type:'post',
                          url: base_url+'controller/visa_passport_ticket/train_ticket/refund/refund_save.php',
                          data:{ train_ticket_id : train_ticket_id, refund_amount : refund_amount, refund_date : refund_date, refund_mode : refund_mode, bank_name : bank_name, transaction_id : transaction_id, bank_id : bank_id, entry_id_arr : entry_id_arr },
